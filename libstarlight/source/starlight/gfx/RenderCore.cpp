    ...
    void ApplyBlendMode(C3D_TexEnv* env, BlendMode mode) {
        switch(mode) {
            case BlendMode::Mask: // multiplies the buffer contents by the mask texture
                ...
                C3D_TexEnvOpRgb(env, GPU_TEVOP_RGB()); // and the rest is the same as blend
                C3D_TexEnvOpAlpha(env, GPU_TEVOP_A(), GPU_TEVOP_A_SRC_ALPHA);
                C3D_TexEnvFunc(env, C3D_TexEnvMode(), GPU_MODULATE);
                C3D_TexEnvFunc(env, C3D_TexEnvMode(), GPU_MODULATE);
                break;

            case BlendMode::Replace:
                C3D_AlphaBlend(GPU_BLEND_ADD, GPU_BLEND_ADD, GPU_ONE, GPU_ZERO, GPU_ONE, GPU_ZERO); // flat replace
                C3D_TexEnvOpRgb(env, GPU_TEVOP_RGB());
                C3D_TexEnvOpAlpha(env, GPU_TEVOP_A());
                C3D_TexEnvFunc(env, C3D_TexEnvMode(), GPU_REPLACE);
                C3D_TexEnvFunc(env, C3D_TexEnvMode(), GPU_REPLACE);
                break;
            ...
            case BlendMode::Blend:
                C3D_AlphaBlend(GPU_BLEND_ADD, GPU_BLEND_ADD, GPU_ONE, GPU_ONE_MINUS_SRC_ALPHA, GPU_ONE, GPU_ONE_MINUS_SRC_ALPHA); // premult
                C3D_TexEnvOpRgb(env, GPU_TEVOP_RGB());
                C3D_TexEnvOpAlpha(env, GPU_TEVOP_A(), GPU_TEVOP_A_SRC_ALPHA); // for color, the second op was 0... but that's the same value so whatever
                C3D_TexEnvFunc(env, C3D_TexEnvMode(), GPU_MODULATE); // and for color, this was REPLACE, not sure if that actually matters
                C3D_TexEnvFunc(env, C3D_TexEnvMode(), GPU_MODULATE);
                break;
        }
    }
...
void RenderCore::BindTexture(C3D_Tex* tex, const Color& color, BlendMode mode) {
    ...
    C3D_TexEnvSrc(env, C3D_Both, GPU_TEXTURE0, GPU_CONSTANT, GPU_TEVSRC());
...
void RenderCore::BindColor(const Color& color, BlendMode mode) {
    C3D_TexEnv* env = C3D_GetTexEnv(0);
    C3D_TexEnvSrc(env, C3D_Both, GPU_CONSTANT, GPU_TEVSRC());
    ApplyBlendMode(env, mode);
    C3D_TexEnvColor(env, color.Premultiplied());
}
...
CRenderTarget::CRenderTarget(int width, int height, bool forceExact) {
    ...
    C3D_RenderTargetClear(tgt, static_cast<C3D_ClearBits>(0), 0, 0);
}
